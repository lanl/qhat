...
def build_case_summary(
    ...
    # Primary cross-case baseline: JW descending-magnitude ordering only.

    # JW signed remains in the input as a secondary diagnostic/control schedule.

    fresh["fresh_jw_magnitude_bch_cancellation_ratio"] = fresh["case_id"].map(

        deterministic_cancellation["jw_magnitude_baseline"]

    )

    fresh["fresh_jw_magnitude_to_signed_bch_cancellation_ratio"] = (

        fresh["fresh_jw_magnitude_bch_cancellation_ratio"]

        / fresh["case_id"].map(deterministic_cancellation[REFERENCE])

    )

    # > 1 means the signed fermionic schedule has the lower Trotter error.

    fresh["fresh_jw_magnitude_to_signed_advantage"] = (

        fresh["fresh_jw_magnitude_one_minus_overlap"]
    ...
    fresh["fresh_observed_outcome"] = np.where(
        ...
        fresh["fresh_jw_magnitude_to_signed_advantage"] > 1.0,
...
def build_matched_pair_summary(case_summary: pd.DataFrame) -> pd.DataFrame:
    ...
    for matched_pair, group in case_summary.groupby("matched_pair", sort=True):
        ...
        values = [
            ...
            favorable_row["fresh_jw_magnitude_to_signed_bch_cancellation_ratio"],

            negative_row["fresh_jw_magnitude_to_signed_bch_cancellation_ratio"],

            favorable_row["fresh_jw_magnitude_to_signed_advantage"],

            negative_row["fresh_jw_magnitude_to_signed_advantage"],
...
def build_validation_statistics(
    ...
    direct = case_summary[
        ...
        & (case_summary["fresh_jw_magnitude_to_signed_bch_cancellation_ratio"] > 0.0)

        & np.isfinite(case_summary["fresh_jw_magnitude_to_signed_advantage"])

        & (case_summary["fresh_jw_magnitude_to_signed_advantage"] > 0.0)

    ]

    direct_x = np.log10(

        direct["fresh_jw_magnitude_to_signed_bch_cancellation_ratio"].to_numpy(float)

    )

    direct_y = np.log10(direct["fresh_jw_magnitude_to_signed_advantage"].to_numpy(float))
    ...
    rows.append(

        {
            ...
            "description": (
                ...
                "JW-magnitude/signed BCH cancellation ratio versus freshly recomputed "

                "JW-magnitude/signed error ratio"
    ...
    absolute_stats = correlation_statistics(
        ...
        np.log10(absolute["fresh_jw_magnitude_to_signed_advantage"].to_numpy(float)),
...
def make_plot(
    ...
    x = np.log10(valid["fresh_jw_magnitude_to_signed_bch_cancellation_ratio"])

    y = np.log10(valid["fresh_jw_magnitude_to_signed_advantage"])
    ...
    for outcome, group in valid.groupby("expected_outcome", sort=True):
        ...
        group_x = np.log10(
            ...
            group["fresh_jw_magnitude_to_signed_bch_cancellation_ratio"]

        )

        group_y = np.log10(group["fresh_jw_magnitude_to_signed_advantage"])
    ...
    axes[1].set_xlabel(r"$\log_{10}(R_{\mathrm{JW,mag}}/R_{signed})$")

    axes[1].set_ylabel(r"$\log_{10}(\epsilon_{\mathrm{JW,mag}}/\epsilon_{signed})$")
...
def write_report(
    ...
    lines = [
        ...
        "from this run's JW-magnitude and signed-reference rows; JW-signed is retained as a secondary diagnostic.",

        "",

        "- JW-magnitude/signed cancellation ratio vs JW-magnitude/signed error ratio: "
