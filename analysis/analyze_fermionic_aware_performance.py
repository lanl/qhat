...
"""Compare fermionic-aware ordering with the JW magnitude-descending baseline.

The input is the exact case-level join produced by
``analyze_jw_magnitude_baseline.py``.

The primary comparator is magnitude-descending Jordan--Wigner ordering for
the same Hamiltonian and numerical settings. Signed-coefficient JW is
...
def classify_outcome(
    ...
    if close_enough(fermionic, jw_magnitude):
        return "tie"

    return (
        "fermionic_win"
        if fermionic < jw_magnitude
        else "jw_magnitude_win"
    )
...
def build_case_table(
    ...
    for _, source in frame.iterrows():
        ...
        if not available:
            ...
            if not finite_positive:
            ...
            elif min(fermionic, magnitude) <= error_floor:
                exclusion_reason = "below_numerical_floor"
                outcome = "below_numerical_floor"

            elif fermionic == 0.0 or magnitude == 0.0:
                ...
                # JW signed is retained only as a secondary diagnostic.
                ratio_signed = (
                    fermionic / signed
                    if math.isfinite(signed) and signed > 0.0
                    else math.nan
                )

                # Fermionic error divided by JW magnitude error.
                ratio_magnitude = fermionic / magnitude

                # Primary advantage metric.
                #
                # A > 1:
                #     fermionic ordering is better.
                #
                # A < 1:
                #     JW magnitude ordering is better.
                advantage = magnitude / fermionic
        ...
        rows.append(
            {
                ...
                "active_space_imbalance": (
                    abs(occupied - vacant) / n_qubits
                ),
                "n_qubits": n_qubits,
                "trotter_steps": int(
                    source["trotter_steps"]
                ),
                "evolution_time": float(
                    source["evolution_time"]
                ),
                "coefficient_tolerance": float(
                    source["coefficient_tolerance"]
                ),
                ...
                "fermionic_to_jw_signed_ratio": (
                    ratio_signed
                ),
                "fermionic_to_jw_magnitude_ratio": (
                    ratio_magnitude
                ),
                "fermionic_advantage_factor": advantage,
                "log10_fermionic_advantage": (
                    log_advantage
                ),
                ...
                "jw_signed_source_csv": source[
                    "jw_signed_source_csv"
                ],
                "jw_magnitude_source_csv": source[
                    "jw_magnitude_source_csv"
                ],
    ...
    cases["global_advantage_rank"] = pd.Series(
        pd.NA,
        index=cases.index,
        dtype="Int64",
    )
...
def summarize_subset(
    ...
    losses = int(
        (
            valid["outcome"] == "jw_magnitude_win"
        ).sum()
    )
    ...
    return {
        ...
        "fermionic_available": int(
            subset["fermionic_available"].sum()
        ),
        ...
        "fermionic_not_available": int(
            (
                subset["exclusion_reason"]
                == "fermionic_not_available"
            ).sum()
        ),
        "fermionic_wins": wins,
        "ties": ties,
        "jw_magnitude_wins": losses,
        "fermionic_win_fraction": (
            wins / len(valid)
            if len(valid)
            else math.nan
        ),
        ...
        "median_fermionic_to_jw_magnitude_ratio": (
            float(
                valid[
                    "fermionic_to_jw_magnitude_ratio"
                ].median()
...
def build_summaries(
    ...
    for molecule, group in cases.groupby(
        "molecule",
        sort=True,
    ):
    ...
    molecule_summary = molecule_summary.drop(
        columns="group"
    )
...
def plot_molecule_overview(
    ...
    for molecule in order:
        ...
        for basis, basis_group in group.groupby(
            "basis",
            sort=True,
        ):
        ...
        q25, median, q75 = values.quantile(
            [0.25, 0.5, 0.75]
        )
    ...
    ax_ratio.set_xlabel(
        r"Fermionic advantage "
        r"$E_{\mathrm{JW,mag}}/E_{\mathrm{F}}$"
    )

    ax_ratio.set_title(
        "(a) Case distribution and median [IQR]"
    )
    ...
    ax_outcome.set_xticklabels(
        ["0", "25", "50", "75", "100"]
    )

    ax_outcome.set_xlabel(
        "Outcome share (%)"
    )
    ...
    figure.suptitle(
        "Fermionic-aware ordering vs "
        "JW descending-magnitude baseline",
        y=1.01,
        fontsize=13,
    )
    ...
    figure.text(
        0.5,
        0.012,
        "Advantage > 1 favors fermionic-aware ordering; "
        "comparisons at or below 1e-12 are excluded.",
        ha="center",
        va="bottom",
        fontsize=8,
    )
...
def plot_molecule_scaling(
    ...
    for index, molecule in enumerate(order):
        ...
        axis.text(
            ...
            (
                "median="
                f"{summary_row['median_fermionic_advantage']:.3g}"
            ),
    ...
    figure.suptitle(
        "Active-space size dependence "
        "of fermionic-aware ordering",
        fontsize=13,
        y=1.0,
    )
...
def main() -> None:
    ...
    molecule_summary, basis_summary = (
        build_summaries(cases)
    )

    condition_summary = (
        build_condition_summary(cases)
    )

    write_csv(
        args.outdir
        / "fermionic_aware_case_performance.csv",
        cases,
    )

    write_csv(
        args.outdir
        / "fermionic_aware_molecule_performance.csv",
        molecule_summary,
    )

    write_csv(
        args.outdir
        / "fermionic_aware_molecule_basis_performance.csv",
        basis_summary,
    )

    write_csv(
        args.outdir
        / "fermionic_aware_condition_performance.csv",
        condition_summary,
    )
    ...
    print(
        f"Input cases: {len(cases)}"
    )

    print(
        "Fermionic-aware rows available: "
        f"{int(cases.fermionic_available.sum())}"
    )

    print(
        "Valid comparisons: "
        f"{int(overall.valid_comparisons)}"
    )

    print(
        "Fermionic wins / ties / JW-magnitude wins: "
        f"{int(overall.fermionic_wins)} / "
        f"{int(overall.ties)} / "
        f"{int(overall.jw_magnitude_wins)}"
    )
